...
function  m = makcell2mat (  c  )
% 
% m = makcell2mat (  c  )
% 
% MET Analysis Kit. This is a MAK version of Matlab's cell2mat (R2015b).
% The only change is that c may contain a cell array in all elements , as
% opposed to cell2mat which will not allow any.
% 
% Written by Jackson Smith - May 2018 - DPAG , University of Oxford
% 
  
  % Checks whether c is actually a cell array
  if  ~ iscell (  c  )
    
    error (  'MAK:makcell2mat:c'  ,  ...
      'makcell2mat: c must be a cell array'  )
    
  end
  
  %CELL2MAT Convert the contents of a cell array into a single matrix.
  %   M = CELL2MAT(C) converts a multidimensional cell array with contents
  %   of the same data type into a single matrix. The contents of C must be
  %   able to concatenate into a hyperrectangle. Moreover, for each pair of
  %   neighboring cells, the dimensions of the cell's contents must match,
  %   excluding the dimension in which the cells are neighbors. This
  %   constraint must hold true for neighboring cells along all of the cell
  %   array's dimensions.
  %
  ...
  %
  %   MAK edit - no error if all elements of c contain cell arrays
  %
  ciscell = false ; % iscell(c{1});
  cisobj = isobject(c{1});
  if cisobj || ciscell
  ...
  % Construct the matrix by concatenating each dimension of the cell array
  %   into a temporary cell array, CT
  % The exterior loop iterates one time less than the number of dimensions,
  %   and the final dimension (dimension 1) concatenation occurs after the
  %   loops
  ...
  for cdim=(length(csize)-1):-1:1
      ...
      % Concatenate the dimension, (CDIM+1), at each element in the
      %   temporary cell array, CT
      for mind=1:prod(cts)
          [mref{1:ctsl}] = ind2sub(cts,mind);
          % Treat a size [N 1] array as size [N], since this is how the
          %   indices are found to calculate CT
          if ctsl==2 && cts(2)==1
              mref = {mref{1}};
          end
